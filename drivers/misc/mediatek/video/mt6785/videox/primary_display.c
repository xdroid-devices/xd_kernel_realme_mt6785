...
static void change_dsi_vfp(struct cmdqRecStruct *handle, unsigned int fps)
{
	...
	DISPMSG("[fps]:%s,cal [fps->vfp] [%d->%d]\n",
		__func__, fps, VFP_PORTCH);

	/*look up table to adjust VFP related to fps*/
	fps_levels = pgc->plcm->params->dsi.dynamic_fps_levels;
	p_fps_table = pgc->plcm->params->dsi.dynamic_fps_table;

	if (p_fps_table == NULL) {
		DISP_PR_INFO("[fps]:%s,no fps table! use cal vfp:%d\n",
			__func__, VFP_PORTCH);
		...
		for (i = 0; i < fps_levels; i++) {
			if (fps == (p_fps_table[i]).fps) {
				...
				DISPMSG("[fps]:%s,LTB [fps->vfp] [%d->%d]\n",
					__func__, fps, vfp_to_set);
				break;
			}
		}
		if (i == fps_levels) {
			DISPCHECK("[fps]:%s,no %d entry, use vfp:%d\n",
				__func__, fps, VFP_PORTCH);
			vfp_to_set = VFP_PORTCH;
		}
	}
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		...
		if (primary_display_is_support_ARR()) {
			CMDQ_VARIABLE trigger_run_time_var;

			cmdq_op_init_variable(&trigger_run_time_var);
			/*check whether the first time run of the loop,
			 * only clear SOF event for the first run;
			 * if clearing SOF for each time run, the next SOF
			 * will may be cleared before waiting,
			 * when GCE is busy at other task ,
			 * the next SOF maybe has arrived
			 * before trigger loop scheduled.
			 */
			cmdq_op_read_reg(pgc->cmdq_handle_trigger,
				pgc->trigger_record_slot,
				&trigger_run_time_var, ~0);
			cmdq_op_if(pgc->cmdq_handle_trigger,
				trigger_run_time_var, CMDQ_EQUAL, 0);
			cmdq_op_add(pgc->cmdq_handle_trigger,
				&trigger_run_time_var,
				trigger_run_time_var, 1);
			cmdq_op_write_reg(pgc->cmdq_handle_trigger,
				pgc->trigger_record_slot,
				trigger_run_time_var, ~0);
			/* wait and clear RDMA0_SOF for vfp change */
			cmdqRecClearEventToken(pgc->cmdq_handle_trigger,
					       CMDQ_EVENT_DISP_RDMA0_SOF);
			cmdq_op_end_if(pgc->cmdq_handle_trigger);
		}
				       ...
				       enum DISP_MODULE_ENUM after_engine)
{
	...
	if (ret) {
		DISP_PR_ERR("dl_to_dc capture:Fail to create cmdq handle\n");
		ret = -1;
		goto out;
	}
	...
	if (ret) {
		DISP_PR_ERR(
			"dl_to_dc capture:Fail to create cmdq wait handle\n");
		ret = -1;
		goto out;
	}
...
_trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	...
	DISPINFO("%s,fps_cng=%d,arr=%d,vdo=%d\n",
		__func__, dynamic_fps_changed,
...
static int _ovl_fence_release_callback(unsigned long userdata)
{
	...
	if (primary_display_is_support_ARR()) {
		cmdqBackupReadSlot(pgc->dsi_vfp_changed, 0, &vfp_changed);

		if (vfp_changed) {
			/*ToDo: ARR send cmd to DDIC*/
			/*when ovl fence release called, means new VFP
			 * will definitely be change after next SOF
			...
			/*update idle timeout*/
			new_idle_inteval =
				primary_display_get_idle_interval(
					pgc->working_dfps);
			old_idle_inteval =
				disp_lp_set_idle_check_interval(
				new_idle_inteval);
			DISPMSG("[fps]:update idle time %dms->%dms\n",
				old_idle_inteval, new_idle_inteval);

			primary_display_notify_fps_chg(pgc->working_dfps);

			cmdqBackupWriteSlot(pgc->dsi_vfp_changed, 0, 0);
			 ...
			 int is_lcm_inited)
{
	...
	if (IS_ERR(init_decouple_buffer_thread))
		DISP_PR_ERR(
		"kthread_run init_decouple_buffer_thread err = %d\n",
...
int primary_suspend_release_fence(void)
{
	...
	for (i = 0; i < DISP_SESSION_TIMELINE_COUNT; i++) {
		DISPDBG(
			"mtkfb_release_layer_fence session=0x%08x,layerid=%d\n",
			session, i);
		mtkfb_release_layer_fence(session, i);
	}
...

	{
		...
		if (primary_display_is_decouple_mode()) {
			if (pgc->ovl2mem_path_handle)
				...
				DISP_PR_ERR(
				"in DC mode but no ovl2mem_path_handle\n");
		}
					  ...
					  int need_merge)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISP_PR_INFO(
			"%s, skip because primary dipslay is slept\n",
			     __func__);
		goto done;
	}
	primary_display_idlemgr_kick(__func__, 0);
	...
	if (cfg->setter != SESSION_USER_INVALID) {
		...
		if (o_mode == DISP_SESSION_DIRECT_LINK_MODE) {
			if (bypass) {
				...
				DISP_PR_INFO(
					"request DC without a rdma frame\n");
...
int primary_display_force_set_vsync_fps(unsigned int fps, unsigned int scenario)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISPCHECK("Sleep State fps change invalid\n");
		ret = -1;
		goto done;
	}
	...
	if (!primary_display_is_support_ARR()) {
		DISPCHECK("[fps]:%s, not support ARR\n", __func__);
		ret = -1;
		goto done;
	}
	if ((fps < primary_display_get_min_refresh_rate()) ||
		(fps > primary_display_get_max_refresh_rate())) {
		DISPCHECK("[fps]: %s,invalid new fps:%d\n", __func__, fps);
		ret = -1;
		goto done;
	}

	DISPMSG("%s:scenario=%d, new request fps=%d\n",
		__func__, scenario, fps);

	mmprofile_log_ex(ddp_mmp_get_events()->primary_chg_fps_req,
			 MMPROFILE_FLAG_START, fps, scenario);
	/* scenario is APP, and support ARR change fps */
	if ((scenario == 0) && primary_display_is_support_ARR()) {
		/* record dynamic fps */
		pgc->dynamic_fps = fps;
		 ...
		 */
		/*kick idle, if is idle, kick idle will use pgc->dynamic_fps
		 * even not in idle, delay enter idle time,
		 * update kick timestamp
		 */
		if (primary_display_is_idle()) {
			DISPMSG("it is idle, kick idle & use dynamic fps:%d\n",
				fps);
			dynamic_fps_changed = 0;
			if (disp_helper_get_option(DISP_OPT_IDLE_MGR))
				primary_display_idlemgr_kick(__func__, 0);
			...
			/*even not idle, delay enter idle time*/
			if (disp_helper_get_option(DISP_OPT_IDLE_MGR))
				primary_display_idlemgr_kick(__func__, 0);
		}
	}
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_chg_fps_req,
			 MMPROFILE_FLAG_END,
			 dynamic_fps_changed, pgc->dynamic_fps);
done:
	_primary_path_unlock(__func__);
	return ret;
}
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	if (ret) {
		DISP_PR_INFO(
			"fail to create primary cmdq handle for backlight\n");
		return -1;
	}
			 ...
			 unsigned int *lcm_value)
{
	...
	if (ret) {
		DISP_PR_INFO(
			"fail to create primary cmdq handle for setlcmcmd\n");
		return -1;
	}
...
UINT32 DISP_GetFBRamSize(void)
{
		...
		if (!fb_size_cal) {
			...
			header_buf_size =
				...
				ALIGN_TO(DISP_GetScreenHeight(),
				MTK_FB_ALIGNMENT)) /
				(OVL_COMPRESS_TILE_W * OVL_COMPRESS_TILE_H) *
				OVL_HEADER_SIZE_PER_TILE_BYTES;
...
/*it is easy to cause screen flicker when stop video mode and read ddic by cpu*/
/*read ddic can be tested by compare id or read ddic esd*/
	if (primary_display_is_video_mode()) {
...
int primary_display_resolution_test(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(width_array); i++) {
		...
		if (ret > 0 && !dpmgr_path_is_busy(pgc->dpmgr_handle)) {
			if (i == 0)
				DISPCHECK(
				"%s:display_result 0x%x unlimited!\n",
					  __func__, array_id[i]);
			else if (i == 1)
				DISPCHECK(
				"%s:display_result 0x%x unlimited (W<H)\n",
					  __func__, array_id[i]);
			else
				DISPCHECK(
				"%s:display_result 0x%x(%d x %d)\n",
					  __func__, array_id[i],
					  dst_width, dst_heigh);
			break;
		}
...
/*---------------- function for fps change start------------------*/
int primary_display_wait_fps_change(unsigned int *new_fps)
{
	int ret = 0;

	DISPMSG("[fps] waits for fps change...\n");
	mmprofile_log_ex(ddp_mmp_get_events()->primary_chg_fps_wait,
			 MMPROFILE_FLAG_START,
			 atomic_read(&primary_display_fps_chg_trigger),
			 pgc->working_dfps);
	ret = wait_event_interruptible(pgc->fps_chg_wait_queue,
		atomic_read(&primary_display_fps_chg_trigger));
	...
	/*should not use dynamic_fps directly,
	 * maybe this is a new ioctl updated value but not worked on DSI
	 */
	*new_fps = pgc->working_dfps;

	DISPMSG("[fps] waits for fps change done,new fps:%d\n", *new_fps);
	mmprofile_log_ex(ddp_mmp_get_events()->primary_chg_fps_wait,
		 MMPROFILE_FLAG_END,
		 atomic_read(&primary_display_fps_chg_trigger),
		 pgc->working_dfps);
...
unsigned int primary_display_get_vfp(unsigned int fps)
{
	...
	/*look up table for vfp related to fps*/
	for (i = 0; i < fps_levels; i++) {
		if (fps == (params->dsi).dynamic_fps_table[i].fps) {
			vfp = (params->dsi).dynamic_fps_table[i].vfp;
			DISPMSG("[fps] %d fps <-> %d vfp\n", fps, vfp);
			break;
		}
	}
	if (i == fps_levels) {
		DISPCHECK("[fps]:%s,no %dfps entry,use %d vfp\n",
			__func__, fps, vfp);
	}
	return vfp;
...
int primary_display_get_dynamic_fps_info(
	struct dynamic_fps_levels *fps_levels)
{
	int ret = 0;
	struct LCM_PARAMS *params;
	unsigned int i = 0;
	...
	for (i = 0; i < tmp_fps_levl; i++) {
		(fps_levels->fps_levels)[i] =
			(params->dsi).dynamic_fps_table[i].fps;
		DISPMSG("%s,fps_levels[%d] = %d\n",
			__func__, i, (fps_levels->fps_levels)[i]);
	}
...
unsigned int primary_display_is_support_ARR(void)
{
	...
	if (disp_helper_get_option(DISP_OPT_ARR_PHASE_1) &&
		primary_display_is_video_mode() &&
		disp_lcm_is_arr_support(pgc->plcm)) {
		DISPDBG("%s,support arr\n", __func__);
		return 1;
	}
	DISPDBG("%s,not support arr\n", __func__);
	return 0;
}
unsigned int primary_display_get_dyn_fps(unsigned int VFP_PORTCH)
{
	...
	for (i = 0; i < fps_levels; i++) {
		if (VFP_PORTCH ==
			...
			fps = (params->dsi).dynamic_fps_table[i].fps;
			DISPMSG("[fps] %d vfp <-> %d fps\n", VFP_PORTCH, fps);
			break;
		}
	}
	if (i == fps_levels) {
		DISPCHECK("[fps]%s, no %d vfp entry, use 60fps\n",
			__func__, VFP_PORTCH);
	}
	return fps;
}

void primary_display_init_arr_fps(void)
{
	unsigned int VFP_PORTCH = pgc->plcm->params->dsi.vertical_frontporch;
	unsigned int fps = primary_display_get_dyn_fps(VFP_PORTCH);
...
void primary_display_notify_fps_chg(unsigned int notify_fps)
{
	/*ToDo: ARR consider whether need lock*/
	if (pgc->fps_chg_last_notify == notify_fps) {
		DISPMSG("%s, equal with last notify %d\n",
			__func__, notify_fps);
		return;
	}
	DISPMSG("[fps]:wake up blocking ioctl waiting for fps change\n");
	/*1,notify to userspace to reset SW vsync*/
	mmprofile_log_ex(ddp_mmp_get_events()->primary_chg_fps_notify,
	 MMPROFILE_FLAG_PULSE,
	 atomic_read(&primary_display_fps_chg_trigger),
	 notify_fps);

	atomic_set(&primary_display_fps_chg_trigger, 1);
	wake_up_interruptible(&(pgc->fps_chg_wait_queue));

	/*2,notify to fpsgo*/
	disp_invoke_fps_chg_callbacks(notify_fps);
	pgc->fps_chg_last_notify = notify_fps;

}

void primary_display_update_vfp_line_slot(
		struct cmdqRecStruct *handle, unsigned int apply_vfp)
{
	cmdqRecBackupUpdateSlot(handle, pgc->dsi_vfp_line, 0, apply_vfp);
	/*if ioctl change fps->enter idle->leave idle->config
	 * vfp will be change during leave idle
	 * no need change vfp during config another time
	 */
	dynamic_fps_changed = 0;
}
					 ...
					 int need_lock)
{
	...
	case WORKING_ARR_DFPS:
		...
		break;
	case HW_CURRENT_FPS:
		fps = pgc->hw_current_fps;
		break;
	case LCM_MAX_FPS:
		fps = (params->dsi).dynamic_fps_table[0].fps;
		break;
	default:
		break;
	}
